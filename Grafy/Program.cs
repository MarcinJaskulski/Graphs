    ...
    class Program
    {
        static void GenerateAdjecencyMatrix(ref int[,] adjacencyMatrix, int saturate, int n) // generuje macierz sąsiedztwa
        {
            ...
            {                          // n-1, bo ostatni wiersz musi być cąły pusty z racji braku 1 na przekątnej i poniżej
                ...
                {
                    i--; // jeśli ten element miał już 1, to zmniejszam i, żeby zgadzało się nasycenie -> w tym przypadku to nie wystąpi
                }
            }

            for (int i = 0; i < saturate-n+1; i++) // dzięki "-n" losujemy od przekątnej + zgadza się nasycenie; +1 bo we wczesniejszej pętli odjeliśmy
            {
        ...
        {
            int lastNode = -1; // będzie zawierał wartość ostatniego węzła, który jest wskaśnikiem do kolejnego miejsca, do zapisu
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    if (item == commingList[i].First()) // Jęsli jest to pierwszy element, to zpisujemy w kolumnie nagłówkowej
                    {
                        grafMatrix[i, n] = lastNode = commingList[i].First(); // Uzupełnienie kolumny nagłówkowej
                    }
                    else if (item == -1) // jeśli jest to już ostatni element
                    {
        ...
        static void AdjecencyBFSSort(ref int[,] adjecencyMatrix, int n) // sortowanie BFS z macierzy sąsiedztwa
        {
            ...
            //szukanie kolejnych miesjc "0" - bez krawędzi -> Iteracja po amountPredecessor
            for(int i=0; sortIndex < n; i++)
            {
                ...
                if (amountPredecessor[i] == 0)
                {
                    ...
                    amountPredecessor[i] = -1; // "Usunięcie" z tabeli -> pokazanie, że te wartości już wykorzystaliśmy

                    // obniżenie w tabeli o 1 -> Przeszukanie połączeń (czyli obniżamy o 1 wszyskie węzły, do których idzie) // Inne dla każdej reprezentacji
                    for (int j=0; j<n; j++)
                    {
        ...
        static void DFS(int n, ref int[,] adjacencyMatrix, ref List<int>[] commingList, ref int[,] EdgeTable, ref int[,] grafMatrix, int m)
        {
            ...
            for (int i = 0; i < n; i++) //dla każdego wiezrchołka
            {
                if (!visited.Contains(i)) EdgeDFSSort(ref EdgeTable, ref sorted, ref visited, i, ref Scounter, ref VCounter, m); //jeśli w visited nie ma wierzchołka, wywołujemy
            }
            sw.Stop();
        ...
        static void Main()
        {
            for (int x = 1;x <= 10; x++)
            {
                ...
                int n = x*500 ; // długość boku -> liczba wierzchołków
